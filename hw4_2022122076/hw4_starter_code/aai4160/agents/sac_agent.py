...
class SoftActorCritic(nn.Module):
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            # if this is rsample, then Q value is also differentiable w.r.t to actor parameter
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        action_distribution: torch.distributions.Distribution = self.actor(obs) # batch_size
        ...
        with torch.no_grad():
            ...
            copied_obs=obs.unsqueeze(0).repeat(self.num_actor_samples,1,1)
            assert copied_obs.shape == (self.num_actor_samples,obs.shape[0],obs.shape[1])
            
            q_values = self.critic(copied_obs,action)
            ...
            #now, q_values have dimension of num_samples*batch_size
        log_probs = action_distribution.log_prob(action)
        # Do REINFORCE (without baseline)
        # TODO(student): Calculate log-probs
        
         # q_values dim = self.num_actor_samples,batch_size
